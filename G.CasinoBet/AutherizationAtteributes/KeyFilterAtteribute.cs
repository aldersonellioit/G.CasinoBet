...
namespace G.CasinoBet.AutherizationAtteributes
{
            ...
            if (body != "")
            {
                try
                {
                    ...
                    if (!CheckWords(json))
                    {
                        ...
                        var _frontservice = requestScope.GetService(typeof(IDimBetService)) as IDimBetService;
                        _frontservice.CheckWords(uu_id.ToString(), uid.ToString(), Commanhelper.GetIPAddress(), JsonConvert.SerializeObject(json), filterContext.Request.RequestUri.AbsolutePath);
